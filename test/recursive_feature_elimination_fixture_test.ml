...
let read_fixture () =
  let fixture = { vectors = Hashtbl.create 12; matrices = Hashtbl.create 6 } in
  In_channel.with_open_text (Sys.getenv "MODELKIT_RFE_FIXTURE") (fun input ->
...
module Ridge_rfe = Recursive_feature_elimination.Make (Ridge_importance)
...
let test_fixture () =
  ...
  in
  let regression =
    Linear_rfe.create ~feature_count:2 (Linear_regression.create ()) |> get
    |> fun specification ->
    Linear_rfe.fit specification
  ...
  in
  let classification =
    Ridge_rfe.create ~step:(Recursive_feature_elimination.Fraction 0.4)
      ~feature_count:2
      (Ridge_classifier.create ~alpha:0.5 () |> get)
    |> get
    |> fun specification ->
    Ridge_rfe.fit specification
      ~rng:(Rng.create (Seed.of_int 0))
  ...
  Alcotest.(check (array int))
    ...
    (Ridge_rfe.selected_indices classification);
  Alcotest.(check (array int))
    "classification ranking"
    (vector fixture "classification_ranking" |> Array.map int_of_float)
    (Ridge_rfe.ranking classification);
  check_vector "classification final importances"
    (vector fixture "classification_importances")
    (Ridge_rfe.final_importances classification);
  check_matrix "classification output"
    (matrix fixture "classification_output")
    (Ridge_rfe.transform classification ~feature_schema:classification_schema
       ~x:classification_x
    |> get)

let () =
  Alcotest.run "Recursive feature elimination reference"
    [ ("sklearn", [ ("dense RFE", `Quick, test_fixture) ]) ]
