...
Every new estimator runs through pipelines, cross-validation, scoring, and grid search, and every metric and solver is checked against committed scikit-learn reference fixtures. Learning-curve training sizes and per-fold train/test scores are likewise checked against `sklearn.model_selection.learning_curve`. The comparative benchmarks under `dev/benchmarks/` are development evidence only; they record convergence parity across data shapes together with a throughput gap on wide designs that later releases will address.

Source checkouts additionally support target-aware pipelines, dense column transformation, feature unions, nested preprocessing chains, expanded splitters, out-of-fold prediction, leakage-safe learning curves, and resumable randomized and successive-halving search. Learning-curve schedules accept absolute counts or fractions of the smallest base training fold, optionally shuffle each fold deterministically, return ordinary per-size cross-validation reports with train and test scores, and can reject an excessive fit plan before fitting. Planned for later versions: validation curves, permutation significance tests, sparse feature input to estimators, artifact codecs for the estimators added since 0.3.2, tree and ensemble models, and accelerated numerical backends. The artifact format remains experimental during 0.x, with a committed golden reader for each released schema.
