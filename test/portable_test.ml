...
let check_float expected observed message =
  if expected <> observed then
    fail
      (Format.asprintf "%s: expected %.17g, observed %.17g" message expected
         observed)

let check_int64 expected observed message =
  if not (Int64.equal expected observed) then
    fail
      (Format.asprintf "%s: expected %Ld, observed %Ld" message expected
         observed)
  ...
  | Error error -> (
      ...
      | Error.Data
          ...
          check
            ...
            "length error has the wrong expectation";
          check
            (observed_observed = observed)
            "length error has the wrong observation"
...
let test_stable_reductions () =
  ...
  check_float 1.0
    (Reference_backend.sum cancellation)
  ...
  check_float 0.0
    (Reference_backend.sum (Vector.of_array [||]))
...
let test_matrix_kernels () =
  let matrix =
    Result.get_ok
      (Matrix.of_arrays [| [| 1e16; 1.0; -1e16 |]; [| 1.0; 2.0; 3.0 |] |])
  in
  let product =
    get_ok
  ...
  in
  check
    (Vector.to_array product = [| 1.0; 6.0 |])
    "matrix-vector product is incorrect";
  let transposed_matrix =
    Result.get_ok
      (Matrix.of_arrays [| [| 1e16; 1.0 |]; [| 1.0; 2.0 |]; [| -1e16; 3.0 |] |])
  in
  ...
  in
  check
    (Vector.to_array transposed_product = [| 1.0; 6.0 |])
    "transposed matrix-vector product is incorrect";
  check_length_mismatch ~name:"dot-product right operand" ~expected:1
    ~observed:0
    (Reference_backend.dot (Vector.of_array [| 1.0 |]) (Vector.of_array [||]));
  check_length_mismatch ~name:"matrix-vector operand" ~expected:3 ~observed:1
    (Reference_backend.matrix_vector_product matrix (Vector.of_array [| 1.0 |]));
...
let test_seed_derivation () =
  ...
  check_int64 2923168321759935908L (Seed.to_int64 nested) "nested seed changed";
  ...
  check_int64 4598690451703514086L
    (Int64.bits_of_float sample)
