...
let read_fixture () =
  let fixture = { vectors = Hashtbl.create 8; matrices = Hashtbl.create 4 } in
  In_channel.with_open_text (Sys.getenv "MODELKIT_UNIVARIATE_FIXTURE")
    (fun input ->
...
let check_float label expected observed =
  let tolerance = 1e-10 *. Float.max 1.0 (Float.abs expected) in
  Alcotest.(check bool)
    label true
    (Float.abs (expected -. observed) <= tolerance)

let check_scores label expected observed =
  Alcotest.(check int)
...
let test_fixture () =
  ...
  in
  let regression =
    Univariate_selection.Regression.create (Univariate_selection.Count 2) |> get
    |> fun specification ->
    Univariate_selection.Regression.fit specification
      ~rng:(Rng.create (Seed.of_int 0))
      ~feature_schema:regression_schema ~x:regression_x ~y:(Some regression_y)
      ()
    |> get
  in
  check_scores "regression F-scores"
    (vector fixture "regression_scores")
    (Univariate_selection.Regression.scores regression);
  Alcotest.(check (array int))
    "regression indices"
    (vector fixture "regression_selected" |> Array.map int_of_float)
    (Univariate_selection.Regression.selected_indices regression);
  check_matrix "regression output"
    (matrix fixture "regression_output")
    (Univariate_selection.Regression.transform regression
       ~feature_schema:regression_schema ~x:regression_x
    |> get);
  ...
  let classification =
    ...
    |> get
    |> fun specification ->
    Univariate_selection.Classification.fit specification
      ~rng:(Rng.create (Seed.of_int 0))
      ~feature_schema:classification_schema ~x:classification_x
      ~y:(Some classification_y) ()
    |> get
  in
  check_scores "classification F-scores"
    (vector fixture "classification_scores")
    (Univariate_selection.Classification.scores classification);
  Alcotest.(check (array int))
    "classification indices"
    (vector fixture "classification_selected" |> Array.map int_of_float)
    (Univariate_selection.Classification.selected_indices classification);
  check_matrix "classification output"
    (matrix fixture "classification_output")
    (Univariate_selection.Classification.transform classification
       ~feature_schema:classification_schema ~x:classification_x
    |> get)

let () =
  Alcotest.run "Univariate selection reference"
    [ ("sklearn", [ ("scores and selected output", `Quick, test_fixture) ]) ]
