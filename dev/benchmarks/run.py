...
def worker_commands(scenario: dict[str, object], scenario_path: Path) -> dict[str, list[str]]:
    ...
    if workload == "preprocessing":
        ...
        modelkit_worker = (
            ...
            / "bench"
            / "ocaml"
            / "transform_cache_worker.exe"
        )
        if not modelkit_worker.exists():
            raise RuntimeError(
                "ModelKit benchmark worker is missing; run `opam exec -- dune "
                "build bench/ocaml/transform_cache_worker.exe`"
            )
        dataset = scenario["dataset"]
        commands["modelkit"] = [
            str(modelkit_worker),
