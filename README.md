...
The reference numerical kernels read immutable Bigarray storage directly and keep their compensated sums in unboxed locals, so dense and CSR products allocate only their output; a property test holds every kernel to an independently written Neumaier fold bit for bit. The `sparse_kernels_v1` development benchmark records the resulting dense-versus-CSR dispatch, one-hot output format, and materialization costs against SciPy, and `solver_shapes_v1` records direct and Newton solver convergence and scale across tall, square, wide, and rank-deficient designs against scikit-learn. Both show convergence and parity across shapes and a throughput gap on wide designs that is documented in `dev/benchmarks/README.md` as the standing performance debt of the linear workbench. The current estimators, datasets, pipelines, and model-selection workflows still accept dense `Matrix.t` feature inputs. `One_hot_encoder.transform_csr` can produce checked sparse output directly, but sparse estimator and workflow integration is scheduled for a later release.
...
The core API retains `Simple_imputer`, `Standard_scaler`, and `Variance_threshold` from 0.3.2. Imputation learns only from the supplied training matrix and treats NaN as the missing-value marker. Scaling uses population variance and maps constant centered features to zero with a scale of one. Variance filtering keeps columns whose variance is strictly greater than its threshold and preserves selected names in input order. These transformers reject infinities with typed errors rather than silently continuing.

ModelKit 0.4.0 adds `Min_max_scaler`, `Max_abs_scaler`, and `Robust_scaler` for fitted per-feature scaling, plus the stateless `Normalizer` for L1, L2, or maximum-norm scaling of each sample. Constant features and zero-norm rows have defined finite behavior, learned statistics remain inspectable, and fitted stages verify their input schema before transforming new data.
