...
- Content-addressed transform-cache foundations provide validated component identities, canonical keys, typed fitted-state codecs, and explicitly scoped, bounded in-memory storage without introducing global mutable state.
...
- **Transform-cache foundations.** Validated package-qualified component identities, deterministic content and fit keys, typed fitted-state codecs, and bounded domain-safe in-memory stores establish the opt-in cache contract. Cache stores are explicit caller-owned values; this increment does not yet connect them to pipelines or persist entries.
